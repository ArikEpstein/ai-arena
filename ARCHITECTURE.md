...
| **Web** | `web/dashboard.template.html` | Self-contained "VS" dashboard, generated by the Arena. |
...
- **Simulated latency is a function of the model, not the prompt** (`MODEL_LATENCY[cfg.model]` plus a
  ≤49 ms answer-length jitter), so a same-model prompt comparison isolates quality with negligible
  latency noise.
...
- In **mock** mode, `mockComplete()` is a deterministic policy driven by a per-runner `MockProfile`
  (`{ chains, reasons, refuses }`). The Arena injects a weak profile for prompt v1 and a strong one for
  v2, so the pass-rate delta is **injected, not derived from prompt text**. This lets the whole harness
  run — and produce a real, non-trivial delta — without any API key. `npm run arena:live` produces a
  genuinely prompt-driven delta.
...
- **Three scenarios:** `prompts` (v1 vs v2, same model — the flagship), `models` (haiku vs sonnet), and
  `iterations` (v1 → v2 → v3).
...
- **Hybrid grading with an LLM-as-judge:** structured expectations (tools, exact values) are graded
  ...
  phrases it differently is no longer a false negative. (In an observed live run, moving the
  open-ended cases from substring matching to the judge lifted the pass-rate from 63% to 100%; live
  numbers depend on the model and are not reproduced in mock CI.)
- **Verdict:** for 2-runner scenarios, classifies the result as a "pure prompt win" (same model, same
  latency, higher pass-rate) or a quality-vs-cost tradeoff.
...
flowchart TD
    ...
    SUM --> V{2 runners?}
    V -->|yes| VERD["verdict → dashboard.html + arena-results.json"]
    V -->|no| PROG["progression log"]
...
| Concern | Choice | Why |
|---|---|---|
| Language | TypeScript 5.6, `strict`, `NodeNext`, `noEmit` | Types are the boundary guard; `NodeNext` enforces `.js` import specifiers |
...
`@anthropic-ai/sdk` is dynamically `import()`-ed inside `liveComplete`/`stream`/`answerWithRag`, so the
mock path never evaluates it.
...
| `SCENARIO` | `prompts` | Arena scenario (`prompts` / `models` / `iterations`) |
